...
5. **Scoring is quality per dollar against the best single model.** `quality = wilson_lcb(accuracy) / acc_best`, `thrift = ref_cost / miner_cost`, and `score = quality^0.9 * thrift^0.1`. A score of 1.0 means "matched the best single model's quality per dollar"; above 1.0 means "beat it". Evidence accumulates per head artifact across epochs, so noise cancels and a lucky epoch does not dethrone. Copied heads are deduplicated — earliest on-chain commitment wins.
